...
| 스킬 | `skill.caveman`, `skill.taste`, `skill.karpathy`, `skill.hallmark` | `npx skills add … --agent universal --copy`로 공유 `.agents/skills`에 **복사**합니다. 그 경로를 10개 CLI가 함께 보므로 한 번 설치로 전부 적용되고, 커밋해서 팀과 나눌 수 있습니다. 상류 Hallmark는 Cursor·Codex용 경로도 따로 안내하지만 CLI별 디렉터리라, 공유 경로 하나가 10개를 함께 덮습니다 |
