# 저장소 범위 AI 에이전트 부트스트랩

**한국어** · [English](AgentSetup-README.md) · [변경 이력](AgentSetup-README-CHANGES.ko.md)
...
수단이므로 **워킹트리가 깨끗할 때만** 동작합니다(추적되지 않는 파일은 세지
않습니다 — 덮어쓸 것이 없기 때문입니다).
...
`Ctrl+F`·`Ctrl+B`·`Ctrl+D`·`Ctrl+Q`는 검색칸·목록 어느 포커스에서도 통합니다 —
...
- **한 파일을 두고 다투는 항목은 라디오로 그려지고 하나만 켜집니다** — 커밋
  ...
  규칙을 함께 밝힙니다(`커밋 메시지 템플릿 · .gitmessage.txt · 하나만
  고릅니다`). 하나를 켜면 형제가 자동으로 꺼지고(검색으로 가려져 있어도
  꺼집니다), 무엇이 해제됐는지 상태줄이 알려 줍니다. `Ctrl+A` 전체 선택도
  묶음에서 하나만 켜며, 이미 고른 판이 있으면 그 선택을 이깁니다.
- **상세 패널이 대상 파일을 먼저 보여 줍니다** — 대상이 하나로 정해진 항목
...
- **탭 안쪽은 성격으로 묶입니다** — 토큰 절감 · 코드 이해 · 디자인 감각 ·
  ...
  **PLUGIN 탭만 설치 범위로 묶입니다** — "저장소 범위 — 이 저장소에만 적용"과
  "머신 전역 — 이 컴퓨터 전체에 적용" 두 헤더로 갈라지고, 행 라벨에
  `(저장소)`/`(전역)` 접미사가, 힌트에 대상 CLI 이름이 붙습니다. 같은 상류의
  두 판(superpowers 등)이 무엇을 건드리는지가 이 탭에서 고르는 기준이기
  때문입니다. 전역 항목을 켜는 순간 상태줄이 그 사실을 알립니다.
...
- 제출하면 적용 직전에 **무엇이 설치/보완/제거되는지 검토 화면**으로 확인한
  ...
  한 번 더 보여 줍니다. 머신 전역 변경이 섞여 있으면 저장소 범위와 갈라
  보여 주고 "이 저장소 밖 설정(~/.codex 등)이 바뀝니다" 경고를 답니다.
  `Esc`로 되돌아갈 수 있습니다.
...
| MCP | `mcp.graft` | stdio 방식 — 상류가 수동 등록용으로 적은 명령 그대로 `npx -y @nanonets/graft mcp`를 쓰므로 따로 설치할 것이 없습니다. 코드베이스 지식 그래프를 도구 6종으로 내며, `graft build`로 그래프를 먼저 만드세요(gitignore되는 `graft/`에 쓰고, 구조 그래프는 LLM 키가 필요 없습니다). 상류 `graft init` 깊은 연동은 `AGENTS.md`·`GEMINI.md`·Claude 훅·`~/.codex`를 직접 고치므로 쓰지 않습니다 — 이 저장소의 부트스트랩이 관리하는 파일들입니다. Windows에서는 의존성 `tree-sitter-kotlin`이 prebuild 없이 C++ 툴체인을 요구하고 Node 24는 컴파일이 안 됩니다(상류 #400) — Node 22 + VS C++ 워크로드가 필요하며, 관리자 머신에서는 서버를 띄우지 못해 배선만 검증했습니다 |
...
| 스킬 | `skill.caveman`, `skill.taste`, `skill.karpathy`, `skill.hallmark`, `skill.diagram-design`, `skill.agent-browser`, `skill.find-skills`, `skill.mcp-builder`, `skill.prompt-master`, `skill.i-have-adhd`, `skill.eli5` | `npx skills add … --agent universal --copy`로 공유 `.agents/skills`에 **복사**합니다. 그 경로를 11개 CLI가 함께 보므로 한 번 설치로 전부 적용되고, 커밋해서 팀과 나눌 수 있습니다. 상류 Hallmark는 Cursor·Codex용 경로도, Diagram Design은 Claude·Codex 플러그인 매니페스트도 따로 두지만 모두 CLI별 디렉터리라, 공유 경로 하나가 11개를 함께 덮습니다. agent-browser는 PATH의 `agent-browser` 바이너리(`npm i -g agent-browser` 후 최초 1회 `agent-browser install`)가 전제이고, mcp-builder는 reference/ 문서와 Apache-2.0 LICENSE.txt가 스킬 디렉터리에 함께 복사됩니다. i-have-adhd는 상류 Claude 마켓플레이스 플러그인 판도 있지만 더 주는 것은 opt-in 상시 적용 플래그용 SessionStart 훅 하나라, 11개 CLI가 함께 보는 공유 스킬로 넣습니다. eli5는 Anthropic 커뮤니티 마켓플레이스 플러그인(`anthropics/claude-plugins-community`, `eli5/` 하위 경로)에 든 스킬 하나입니다 — `/eli5 <주제>`가 HTML 그림 설명서를 만들며, 플러그인이 훅·명령을 더 주지 않아 공유 스킬로 넣습니다 |
...
| 스킬 | `skill.officecli` | Word·Excel·PowerPoint(.docx·.xlsx·.pptx) 문서를 읽고 고치고 만드는 스킬을 상류 `skills/officecli` 폴더에서 공유 `.agents/skills`로 복사합니다(저장소 루트에도 `SKILL.md`가 있어 루트를 가리키면 레지스트리가 저장소 전체 94MB를 복사합니다 — 실측). 스킬이 `officecli` 바이너리를 구동하므로 PATH에 있는 것이 전제입니다 — `npm install -g @officecli/officecli` 또는 상류 `install.sh`·`install.ps1`. Microsoft Office 설치는 필요 없습니다 |
