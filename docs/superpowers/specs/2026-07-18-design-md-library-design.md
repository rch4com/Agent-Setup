...
> **대화형 UI는 이후 대체되었다.** 아래 "대화형 흐름"의 중첩 메뉴(모드 선택 →
> 어떻게 찾을까요 → 제공자 → 카테고리 → 멀티셀렉트)는
> [2026-07-19 TUI 설계](2026-07-19-installer-tui-design.md)의 단일 리스트 화면으로
> 바뀌었다. 비대화형 경로(`--list`/`--set`/`--sync`/`--preview`)와 항목 메커니즘·
> 프로바이더·디렉터리 스캔은 이 문서 그대로 유효하다.
