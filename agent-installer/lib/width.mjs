...
//
// 결합 문자(악센트·변형 선택자·폭 없는 문자)는 0칸이다 — 1로 세면 `é`가
// 두 칸으로 읽혀 열이 왼쪽으로 밀린다. 이모지는 2칸이다 — DESIGN.md의
// 라벨·설명에 실려 오는데 1로 세면 그 행만 오른쪽으로 밀린다. U+2600 구간의
// 기호(☀ 등)는 터미널마다 폭이 갈리는 모호 폭이라 넣지 않는다.
function charWidth(cp) {
  if ((cp >= 0x0300 && cp <= 0x036f) // 결합 분음 부호
    || (cp >= 0x1ab0 && cp <= 0x1aff) || (cp >= 0x1dc0 && cp <= 0x1dff)
    || (cp >= 0x20d0 && cp <= 0x20ff) // 기호용 결합 부호
    || (cp >= 0xfe00 && cp <= 0xfe0f) // 변형 선택자
    || (cp >= 0xfe20 && cp <= 0xfe2f)
    || (cp >= 0x200b && cp <= 0x200f) // 폭 없는 공백·결합자·방향 표식
    || cp === 0xfeff) return 0
  if ((cp >= 0x1f300 && cp <= 0x1f64f) // 이모지 그림·표정
    || (cp >= 0x1f680 && cp <= 0x1f6ff) // 교통·지도
    || (cp >= 0x1f900 && cp <= 0x1f9ff) // 보충 이모지
    || (cp >= 0x1fa70 && cp <= 0x1faff)) return 2
