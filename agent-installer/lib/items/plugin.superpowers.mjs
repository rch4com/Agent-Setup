...
// 상류(obra/superpowers v6.2.0)는 Codex·Antigravity·Copilot CLI 등 11개
// 하니스를 지원하지만 설치 명령이 하니스마다 전부 달라 공유 경로가 없다
// (README: "install Superpowers separately for each one"). 이 항목이 배선하는
// 것은 Claude Code 플러그인 판뿐이다. Kilo·Kiro·Grok·VS Code는 상류에 공식
// 설치 경로가 없다(2026-08-02 검증).
const SEPARATE = ['codex', 'gemini', 'opencode', 'kimi', 'copilot']
