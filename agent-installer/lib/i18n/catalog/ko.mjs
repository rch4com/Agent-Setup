...
export default {
  ...
  'item.global.superpowers.note': 'superpowers를 하니스별 공식 명령으로 사용자 전역에 설치합니다 — codex·copilot은 마켓플레이스, gemini는 extensions, opencode는 전역 opencode.json. 이 머신에 없는 CLI는 건너뜁니다. skill.superpowers와 함께 켜면 겹치는 CLI는 같은 스킬을 두 곳에서 봅니다',
  'item.global.ponytail.note': 'Ponytail을 하니스별 공식 명령으로 사용자 전역에 설치합니다 — codex·copilot은 마켓플레이스(DietrichGebert/ponytail), gemini는 extensions. codex는 설치 후 /hooks에서 훅 등록을 한 번 확인해야 합니다. 상류는 제거 전 scripts/uninstall.js로 ~/.config/ponytail 정리를 안내합니다 — 이 항목은 플러그인만 제거합니다',
  ...
  'item.skill.superpowers.note': '공유 .agents/skills에 스킬 14종을 복사해 10개 CLI가 함께 봅니다. 플러그인 판과 하나만 고를 수 있습니다 — 이쪽은 session-start 훅이 없어 대화 시작 시 자동 주입이 되지 않고, 스킬을 불러서 씁니다',
  'item.skill.agent-browser.note': '브라우저 자동화 CLI를 쓰게 하는 스킬. PATH에 agent-browser 바이너리 필요 — npm i -g agent-browser 후 최초 1회 agent-browser install(Chrome for Testing 다운로드). SKILL.md는 안내 스텁이라 실제 지침은 설치된 CLI가 제공합니다. 본문이 하네스 중립이라 10개 CLI가 함께 씁니다(allowed-tools 줄만 Claude 문법)',
  'item.skill.find-skills.note': '공개 스킬 레지스트리를 검색해 설치를 안내하는 스킬. npx skills 명령만 지시하는 하네스 중립 문서라 10개 CLI가 함께 씁니다',
