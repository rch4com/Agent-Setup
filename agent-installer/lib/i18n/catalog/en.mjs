...
export default {
  ...
  'usage.status': [
    ...
    'the running tool version side by side. Changes nothing. Asks the npm',
    'registry for the latest version (5-second limit; offline just leaves it blank).',
    '',
    'Options:',
  ...
  'error.tomlInvalid': 'Cannot read {path} — invalid TOML: {message}. Fix the file by hand; nothing was written.',
  ...
  'log.skill.relinkDangling': 'warning: {path} is a link whose target no longer exists (the repository moved?). Recreating it.',
  ...
  'item.unsupported.ponytailUser': 'upstream installs it per user (codex/copilot/grok plugin, gemini extension)',
  'item.unsupported.ponytailRules': 'no plugin mechanism — copy an upstream rules file by hand (.kiro/steering/ for Kiro, .github/copilot-instructions.md for VS Code Copilot, AGENTS.md otherwise)',
  'item.unsupported.eccCodexHome': 'upstream ships a native Codex plugin, but it installs into the single active CODEX_HOME — there is no project scope',
  ...
  'item.global.noCli': 'CLI not on this machine: {list} — excluded from the target set',
  'run.keepUserScope': '· {id} is a user-global item and is not removed by omission from --set — uncheck it in the TUI to remove it',
  'item.global.skipped': 'skipped (no CLI): {list}',
  ...
  'item.mcp.graft.note': 'serves a codebase knowledge graph as six MCP tools (find_code, trace_calls, file_api and more). Runs as npx -y @nanonets/graft mcp, so nothing to install, but build the graph (graft/) first with graft build (the structural graph needs no LLM key). On Windows the tree-sitter-kotlin dependency needs a C++ toolchain to compile and Node 24 cannot compile it at all (upstream #400) — use Node 22 plus the VS C++ workload. The upstream graft init edits AGENTS.md, GEMINI.md and hooks directly, so it is not used',
  ...
  'item.skill.officecli.note': 'copies a skill for reading, editing and creating Word, Excel and PowerPoint documents (.docx, .xlsx, .pptx) into the shared .agents/skills. The skill drives the officecli binary, so officecli is required on PATH — npm install -g @officecli/officecli or the upstream install.sh/install.ps1. No Office installation needed',
  ...
  'error.globalExec': 'a harness command failed — {detail}',
  'error.globalOpencodePlugin': '{file}: the plugin key is not an array. Leaving it alone — add "{entry}" by hand.',
  'error.registrySkillInstall': 'skill registry install failed ({skill}): {output}',
  'error.registrySkillAnchor': "{id}: when skill is '*', an anchor skill must be named for detect to look at",
  ...
  'tui.actionFailed': 'The action failed: {message}',
  ...
  'tui.exclusiveSwitched': 'Switched to {kept} — they compete for the same slot, so {dropped} was cleared.',
