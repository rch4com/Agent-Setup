...
// 대화형 화면은 TUI가 맡는다 — 의존성 없이 도는 부트스트랩과 달리 여기는 lib/에 닿는다.
async function loadTui() {
  return withDeps(() => import('./lib/tui/run.mjs'))
}
...
// --list / --set 전용 비대화형 경로. 대화형은 runTui가 맡는다.
...
async function main() {
  ...
  if (argv[0] === 'design') {
    ...
    if (opts.interactive) {
      const { runTui } = await loadTui()
      await runTui(root, { dryRun: opts.dryRun, designDirs: opts.designDirs })
      return
    }
  ...
  if (listOnly || setArg !== null) {
    await runClassic(root, { dryRun, listOnly, setArg })
    return
  }

  const { runTui } = await loadTui()
  await runTui(root, { dryRun, designDirs: collectValues(argv, '--design-dir') })
}
