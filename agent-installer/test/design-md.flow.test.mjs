...
test('--list는 카탈로그를 상태와 함께 출력', async () => {
  ...
  await runDesign(makeTempRepo(), { list: true, log: cap.log, fetchImpl: fileFetch('x'), t: KO })
...
test('--preview 알 수 없는 이름은 예외 없이 안내', async () => {
  const opener = recordingOpener()
  const cap = makeCapture()
  await runDesign(makeTempRepo(), { preview: 'stripe,zzz', opener, log: cap.log, fetchImpl: fileFetch('x'), t: KO })
...
test('sync=installed는 설치본을 재다운로드한다', async () => {
  ...
  await runDesign(root, { sync: 'installed', fetchImpl: fileFetch('# v2'), log: cap.log, t: KO })
...
test('sync=stale은 변경된 설치본을 감지', async () => {
  ...
  await runDesign(root, { sync: 'stale', fetchImpl: fileFetch('# remote'), log: cap.log, t: KO })
...
test('sync=stale은 동일하면 최신으로 보고', async () => {
  ...
  await runDesign(root, { sync: 'stale', fetchImpl: fileFetch(bundled), log: cap.log, t: KO })
...
test('sync=catalog dry-run은 저장 없이 개수만 리포트', async () => {
  ...
  await runDesign(makeTempRepo(), { sync: 'catalog', dryRun: true, fetchImpl, log: cap.log, t: KO })
...
test('makeOpener는 열 수 없는 대상을 실행 없이 거부한다', () => {
  ...
  // output은 이제 구조화 메시지다 — 표시하는 쪽이 toText로 지역화한다.
  const r = makeOpener(false, () => {})('javascript:alert(1)')
  assert.equal(r.ok, false)
  assert.equal(r.output.key, 'design.notOpenable')
  assert.match(toText(KO, r.output), /열 수 있는 대상이 아닙니다/)
})

test('openPreview: webUrl 없으면 안내하고 실패', () => {
  const cap = makeCapture()
  const r = openPreview(() => ({ ok: true }), { label: 'X', webUrl: null }, cap.log, KO)
  assert.equal(r.ok, false)
