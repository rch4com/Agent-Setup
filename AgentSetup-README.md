...
대화형 화면은 **탭 하나짜리 단일 리스트**입니다. 작업·PLUGIN·MCP·SKILL·
DESIGN.MD 탭을 오가며 고르고, 마지막에 한 번 제출해 **일괄 적용**합니다.

| 키 | 하는 일 |
|---|---|
| `Space` | 커서 항목 선택/해제 (검색 중에도 동일 — 언제나 선택입니다) |
| `Tab` / `Shift+Tab` / `←` `→` | 탭 이동 (선택은 탭을 넘나들며 누적됩니다) |
| `↑` `↓` / `PgUp` `PgDn` | 커서 이동 |
| `Enter` | 작업 탭에서는 그 작업 실행, 그 밖에는 **제출**(검토 화면 → 일괄 적용) |
| `Ctrl+A` | 지금 탭에서 보이는 항목 전체 선택/해제 |
| `/` 또는 아무 글자 | 검색 시작 (`Enter` 확정 · `Esc` 해제). 대화형 검색은 한 단어입니다 |
| `Ctrl+O` | 커서 항목 미리보기 (design.md는 브라우저) |
| `Esc` | 검색 해제 → 한 번 더 누르면 종료 |

- 검색은 활성 탭 안에서 걸리지만, **탭 줄에 탭별 적중 수**가 함께 표시되어
  다른 탭의 결과를 놓치지 않습니다(`DESIGN.MD 19/76`).
- 제출하면 적용 직전에 **무엇이 설치/보완/제거되는지 검토 화면**으로 확인한 뒤
  `Enter`로 일괄 적용합니다. `Esc`로 되돌아갈 수 있습니다.
