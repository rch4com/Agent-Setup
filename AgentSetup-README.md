...
| Skill | `skill.caveman`, `skill.taste`, `skill.karpathy`, `skill.hallmark` | **Copied** into the shared `.agents/skills` with `npx skills add … --agent universal --copy`. All 10 CLIs look at that path, so one install applies to all of them, and it can be committed and shared with the team. Upstream Hallmark also documents Cursor and Codex paths of its own, but they are per-CLI directories — the shared path covers all 10 at once |
