...
| Skill | `skill.caveman`, `skill.taste`, `skill.karpathy`, `skill.hallmark`, `skill.diagram-design`, `skill.agent-browser`, `skill.find-skills`, `skill.mcp-builder`, `skill.prompt-master`, `skill.i-have-adhd` | **Copied** into the shared `.agents/skills` with `npx skills add … --agent universal --copy`. All 11 CLIs look at that path, so one install applies to all of them, and it can be committed and shared with the team. Upstream Hallmark documents Cursor and Codex paths of its own, and Diagram Design ships Claude and Codex plugin manifests, but those are all per-CLI directories — the shared path covers all 11 at once. agent-browser presumes the `agent-browser` binary on PATH (`npm i -g agent-browser`, then `agent-browser install` once); mcp-builder's reference/ documents and Apache-2.0 LICENSE.txt are copied along inside the skill directory. i-have-adhd also has an upstream Claude marketplace plugin whose only extra is a SessionStart hook for an opt-in always-on flag; the shared skill is used instead so all 11 CLIs see it |
