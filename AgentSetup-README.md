...
| Skill | `skill.caveman`, `skill.taste`, `skill.karpathy` | **Copied** into the shared `.agents/skills` with `npx skills add … --agent universal --copy`. All 10 CLIs look at that path, so one install applies to all of them, and it can be committed and shared with the team |
